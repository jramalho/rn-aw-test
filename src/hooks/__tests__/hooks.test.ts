...
jest.mock('react-native', () => {
  ...
  return {
    ...RN,
    Keyboard: {
      addListener: jest.fn((_event, _callback) => ({
        remove: jest.fn(),
      })),
    },
    AppState: {
...
describe('Custom Hooks', () => {
  ...
  describe('useAsyncStorage', () => {
    ...
    it('loads stored value from AsyncStorage', async () => {
      ...
      const { result } = renderHook(() =>
        useAsyncStorage('test-key', { data: 'initial' }),
      );
    ...
    it('works with complex object types', async () => {
      ...
      const updatedValue: ComplexType = {
        id: 2,
        name: 'Updated',
        tags: ['c', 'd', 'e'],
      };
      ...
      const { result } = renderHook(() =>
        useAsyncStorage<ComplexType>('complex-key', initialValue),
      );
