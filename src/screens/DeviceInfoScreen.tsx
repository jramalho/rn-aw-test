...
export default function DeviceInfoScreen() {
  ...
  const loadDeviceInfo = async () => {
    try {
      const [device, battery, storage, biometric, loc, tz, haptics] =
        await Promise.all([
  ...
  const batteryStatus = batteryInfo
    ? DeviceInfo.getBatteryStatus(batteryInfo)
    : null;
  const storagePercentage = storageInfo
    ? DeviceInfo.getStorageUsagePercentage(storageInfo) / 100
    : 0;
        ...
        <Card style={styles.card}>
          ...
          <Card.Content>
            ...
            <View style={styles.chipContainer}>
              {deviceInfo.isTablet && (
                <Chip icon="tablet" style={styles.chip}>
                  Tablet
                </Chip>
              )}
              {deviceInfo.isEmulator && (
                <Chip icon="desktop-tower" style={styles.chip}>
                  Emulator
                </Chip>
              )}
              {hasBiometric && (
                <Chip icon="fingerprint" style={styles.chip}>
                  Biometric
                </Chip>
              )}
              {hasHaptics && (
                <Chip icon="vibrate" style={styles.chip}>
                  Haptics
                </Chip>
              )}
            </View>
      ...
      <Card style={styles.card}>
        ...
        <Card.Content>
          <Text variant="bodyMedium" style={styles.noteText}>
            This screen demonstrates a custom TurboModule built with the React
